...
def main():
    ...
    for knjiga in knjige:  # rows from CSV list : knjige
        cur.execute(
            "INSERT INTO knjige (autor_id, naslov, cena, godina) VALUES (%s, %s, %s, %s)",
            (knjiga["autor_id"], knjiga["naslov"], knjiga["cena"], knjiga["godina"]),
        )
    ...
    for i, red in enumerate(svi_redovi):
        ...
        starost = godine[i]
        cur.execute(
            "UPDATE knjige SET starost_knjige = %s WHERE id = %s", (starost, idd)
        )
